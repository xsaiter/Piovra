...
namespace Piovra.Ds {
    public class PriorityQueue<T> where T : IComparable<T> {
        ...
        public static PriorityQueue<T> Max(int capacity = INITIAL_CAPACITY) {
            return new PriorityQueue<T>(new BinaryHeap<T>(capacity, true));
        }

        public static PriorityQueue<T> Min(int capacity = INITIAL_CAPACITY) {
            return new PriorityQueue<T>(new BinaryHeap<T>(capacity, false));
        }
