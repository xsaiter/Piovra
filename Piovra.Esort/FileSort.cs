...
public static class FileSort {
    ...
    static async Task<List<Stream>> SplitFileAsync(string srcFile, int memorySize, string outDir, CancellationToken cancellationToken) {
        EnsureDir(outDir);
    ...
    static async Task MergeAsync(IEnumerable<Stream> streams, string destFile, CancellationToken cancellationToken) {
